...
function validarDatosPersonales() {
	...
	fechaUsuario = document.getElementsByName("fechaUsuario")[0];
	idFechaUsuario = "error" + fechaUsuario.getAttribute("name");
	...
	if (validarVacio(fechaUsuario.value, idFechaUsuario)) {
		avanzar = false;
	}
...
function validarForm(){
	...
	if($('#datosPersonales').is(':visible')){		
		...
		if($('#datosUsuario').is(':visible')){
			mostrarDatosAuto(); //valida datos del usuario
		}
		else{
			if($('#datosAuto').is(':visible')){
				if(mostrarDatosUsuario()!==true){mostrarDatosPersonales();return false;}; //valida datos Personales
				if(enviar=mostrarDatosAuto()!==true){return false}; //valida datos del usuario
				enviar=true;
			}
		}
	};
